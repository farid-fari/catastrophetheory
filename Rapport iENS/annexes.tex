...
\end{thm}
\begin{proof}
	On se ramène à un voisinage de 0 où $df_0=Id_E$ (et donc $F=E$) en prenant pour fonction $df_a^{-1}(f(a+x)-f(a))$ que l'on notera $f$.
	Sur $B_r$, on a $\Vert df_x-Id_E \Vert \leq \frac{1}{2}$. On peut alors écrire $df_x=Id_E-u$ et donc avoir $df_x^{-1}=Id_E+u+u^2+...$ (ça converge puisque la norme est subordonnée).

	On pose, pour $y$ fixé de $B_{\frac{r}{2}}$, $h:x\mapsto x+y-f(x)$. On a par les acroissements finis sur $B_r, \Vert h(x) - h(x')\Vert\leq\frac{\Vert x-x' \Vert}{2}$, donc h contractante.
	Elle est de plus à images dans $B_r$ donc y admet un unique point fixe: $y$ admet un antécédent unique par $f$ dans $B_r$. $f$ est donc bijective $f^{-1}(B_\frac{r}{2}) \cap B_r \to B_{\frac{r}{2}}$.
...
%Si $f\in\cinf(\R^n\times\R^m,\R^p)$, et que le noyau de $df_{(x,y)}$ est localement le graphe d'une fonction $y=g(x)$, alors le noyau de $f$ est localement le graphe d'une application $y=h(x)$.

Plus formellement, si l'on note $f_2(y) = f(x_0, y)$, et que $df_{2,y_0}$ est difféomorphisme local (donc $f_2$ est localement inversible), alors il existe une application $\phi_c$ telle que $f(x,\phi_c(x))=c$ sur un voisinage de $(x_0,y_0)$, et ce pour tout $c$ au voisinage de $f(x_0,y_0)$.
...
De plus, les démonstrations de chacun de ces théorèmes se déduisent facilement de l'énoncé de l'autre: les énoncés sont équivalents.
...
\section{Densité, négligeablité}

\subsection{Généricité}
\begin{defn}
	Une propriété sur des éléments d'un espace topologique $E$ est dité \textbf{générique} si elle est vérifiée sur une intersection d'ouverts denses.
\end{defn}

\begin{defn}
	Un ensemble est dit negligeable s'il est contenu dans une ré
\end{defn}

\subsection{Théorème de Baire}
\begin{thm}{de Baire}
	Dans un espace complet, une intersection d'ouverts denses est dense.
\end{thm}

\section{Courbes de $\R^3$}
...
\begin{proof}[Démonstration (deux premiers points)]
	...
	Quant à la réecriture au voisinage d'un point critique, il faut distinguer les cas selon $\frac{\partial^2 f}{\partial z^2}$ nul ou non, et utiliser la remarque précédente et le théorème de décomposition.
\end{proof}
