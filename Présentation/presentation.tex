...
\begin{frame}{Intersections transverses}
    \begin{onlyenv}<1-3,7>
        \uncover<1-3>{
            ...
            Caractérisations:
            \begin{enumerate}
                \item<2-> Si $dim(F)+dim(G) \geq n$, intersection minimale
                \item<2-> Toutes translations des espaces s'intersectent
...
\begin{frame}{Le théorème de transversalité de \textsc{Thom}}
    ...
    \uncover<2>{\textbf{Exemple 1}: $\{\frac{\partial f}{\partial z}=0\}$ est une variété de codimension $1$ de $J^1(\R^3,\R) \simeq \R^7$}
